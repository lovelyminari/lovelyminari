...
print len(candidates), sorted(candidates, key=operator.itemgetter(1), reverse=True)
...
for candidate in candidates:
    for rank in textrank:
        if candidate[0] == rank[0]:
            candidate.append(rank[1])
            final_keywords.append(candidate)

    if len(candidate) == 3:
        candidate.append(0.01)
        final_keywords.append(candidate)

print 'final keywords : ', len(final_keywords), sorted(final_keywords, key=operator.itemgetter(1), reverse=True)
...
sortedByImportance = sorted(modifiedKeywords, key=operator.itemgetter(3), reverse=True)
print 'Sorting by importance : ', sortedByImportance
sortedByFrequency = sorted(modifiedKeywords, key=operator.itemgetter(1), reverse=True)
